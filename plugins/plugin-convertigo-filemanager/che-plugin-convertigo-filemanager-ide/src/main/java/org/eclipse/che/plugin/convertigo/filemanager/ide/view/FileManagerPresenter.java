...
import org.eclipse.che.plugin.convertigo.filemanager.ide.common.EditorEvent;
...
public class FileManagerPresenter extends BasePresenter implements FileManager.ActionDelegate, FileEventHandler {
	...
	public void onFileOperation(FileEvent event) {
		...
		if (event.getOperationType() == FileOperation.SAVE) {
			...
			if (file.getName().startsWith(EditorFilePrefix.c8o_JscriptStepEditor.toString())) {
				eventName = EditorEvent.save_JscriptStep.toString();
			}
			// save handler transaction
			else if (file.getName().startsWith(EditorFilePrefix.c8o_JscriptTransactionEditor.toString())) {
				eventName = EditorEvent.save_JscriptTransaction.toString();
			}
